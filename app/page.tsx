...
export default function Home() {
  ...
  useEffect(()=>{try{const ids=JSON.parse(localStorage.getItem('saint-pierre-cart-v1')||'[]');setCartCount(Array.isArray(ids)?ids.length:0)}catch{}},[]);
  ...
  useEffect(()=>{setVisibleCount(PAGE_SIZE)},[selectedGrade,selectedState,selectedSubject,selectedCollection,query]);
  const visible=shown.slice(0,visibleCount);
  return <main id="main-content"><VisitTracker/>
    ...
    <header className="site-header">
      ...
      <nav className={menuOpen ? "nav open" : "nav"} aria-label="Main navigation">{site.navigation.map((n,i)=>n.label==='Cart'?<a key={i} href={n.url} onClick={()=>setMenuOpen(false)}>{n.label}{cartCount>0&&<span className='cart-badge'>{cartCount}</span>}</a>:<a key={i} href={n.url} onClick={()=>setMenuOpen(false)}>{n.label}</a>)}</nav>
    ...
    <section className="shop-section" id="shop">
      ...
      <div className="shop-controls"><label>Grade or course<select value={selectedGrade} onChange={e=>setSelectedGrade(e.target.value)}>{gradeLevels.filter(g=>g==='All Levels'||products.some(p=>gradesFor(p).includes(g))).map(g=><option key={g}>{g}</option>)}</select></label><label>Subject<select value={selectedSubject} onChange={e=>setSelectedSubject(e.target.value)}>{['All Subjects',...new Set(products.map(p=>p.subject))].map(v=><option key={v}>{v}</option>)}</select></label><p>PDFs and editable PowerPoints · Contents and availability shown per resource</p></div>{catalogMessage&&catalogMessage.startsWith('Loading')
        ?<div className="product-grid skeleton-grid">{[...Array(6)].map((_,i)=><div key={i} className="product-card skeleton-card"><div className="skeleton-img"/><div className="skeleton-body"><div className="skeleton-line short"/><div className="skeleton-line"/><div className="skeleton-line"/><div className="skeleton-line medium"/></div></div>)}</div>
        :<p role="status" className="catalog-status">{catalogMessage||(shown.length>PAGE_SIZE?`Showing ${visible.length} of ${shown.length} matching resources`:`${shown.length} matching resources`)}</p>}
      <div className="product-grid">{visible.map(p=><article className="product-card" key={p.id}><a className="product-image-frame" href={`/products/${p.id}`} aria-label={`View ${p.title}`}><img width="1160" height="1500" loading="lazy" src={p.thumbnailSrc||`/product-thumbnails/${p.id}.png?v=20260915b`} alt={`${p.title} cover`}/></a><div className="product-info"><p>{p.subject} • {p.level}</p><h3>{p.title}</h3><p className="description">{p.description}</p><p className="resource-facts">{p.pages} pages · {p.formats.join(' + ')} · {p.minutes}</p><a className="sample-link" href={`/products/${p.id}#preview`}>Preview sample page</a><div className="product-footer"><strong>{(p.priceCents/100).toLocaleString('en-US',{style:'currency',currency:'USD'})}</strong><a className="portal-button" href={`/products/${p.id}`}>View resource</a></div><small>{liveOpen&&p.ready?'Download after verified payment.':'Preview only · Not available for purchase yet.'}</small></div></article>)}</div>{shown.length>visible.length&&<div className="load-more-row"><button className="primary-button" onClick={()=>setVisibleCount(c=>c+PAGE_SIZE)}>Load more resources ({shown.length-visible.length} remaining)</button></div>}{!catalogMessage&&shown.length===0&&<p className="empty">No listed resources match these filters. <button onClick={clearFilters}>Clear filters</button> or <a href="/requests">request a resource</a>.</p>}
    </section>
