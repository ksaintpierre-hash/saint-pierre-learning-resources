...
globalThis.__storeEnv={DB:await mf.getD1Database('DB'),PAID_FILES:await mf.getR2Bucket('PAID_FILES'),PAYPAL_MODE:'sandbox',PAYPAL_CLIENT_ID:'test-id',PAYPAL_CLIENT_SECRET:'test-fixture-not-a-secret',PAYPAL_WEBHOOK_ID:'test-webhook'};
...
assert.equal(releaseBeforeProvision.filter(p=>p.ready).length,140);
await s.provision();
const D=globalThis.__storeEnv.DB;
const id='aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa';
let remote,creates=0,captures=0;
const realFetch=globalThis.fetch;
globalThis.fetch=async(url,opts={})=>{if(String(url).endsWith('/v1/oauth2/token'))return Response.json({access_token:'test-token'});if(String(url).endsWith('/verify-webhook-signature'))return Response.json({verification_status:JSON.parse(opts.body).transmission_sig==='valid'?'SUCCESS':'FAILURE'});if(String(url).endsWith('/v2/checkout/orders')&&opts.method==='POST'){creates++;const input=JSON.parse(opts.body);remote={id:'PAYPALORDER123456',status:'APPROVED',purchase_units:input.purchase_units};return Response.json(remote)}if(String(url).endsWith('/capture')){captures++;remote.status='COMPLETED';remote.purchase_units[0].payments={captures:[{id:'CAPTURE123456789',status:'COMPLETED',amount:{currency_code:'USD',value:'5.00'}}]};return Response.json(remote)}if(String(url).includes('/v2/checkout/orders/'))return Response.json(remote);throw new Error('Unexpected outbound request')};
...
test('successful payment and duplicate capture fulfill once',async()=>{await s.captureOrder(req('capture',{orderId:remote.id}));await s.captureOrder(req('capture',{orderId:remote.id}));assert.equal(captures,1);const row=await D.prepare('SELECT * FROM store_orders').first();assert.equal(row.status,'completed');assert.equal(row.fulfillment,'available')});
...
function event(type,id,resource,sig='valid'){return new Request('https://store.example/api/store/webhook',{method:'POST',headers:{'paypal-auth-algo':'SHA256withRSA','paypal-cert-url':'https://api.paypal.com/cert','paypal-transmission-id':'test','paypal-transmission-sig':sig,'paypal-transmission-time':new Date().toISOString()},body:JSON.stringify({id,event_type:type,resource})})}
test('bad signature is rejected, duplicate webhook is recorded once',async()=>{const resource={id:'CAPTURE123456789',supplementary_data:{related_ids:{order_id:remote.id}}};assert.equal((await s.safe(()=>s.webhook(event('PAYMENT.CAPTURE.COMPLETED','bad',resource,'invalid')))).status,400);await s.webhook(event('PAYMENT.CAPTURE.COMPLETED','complete1',resource));await s.webhook(event('PAYMENT.CAPTURE.COMPLETED','complete1',resource));assert.equal((await D.prepare("SELECT COUNT(*) n FROM store_webhooks WHERE id='complete1'").first()).n,1)});
test('refund revokes access and late completed event cannot restore it',async()=>{const {token}=await(await s.downloadToken(req('download-token',{productId:'ca-grade4-multidigit'}))).json();await s.webhook(event('PAYMENT.CAPTURE.REFUNDED','refund1',{id:'REFUND123',status:'COMPLETED',amount:{currency_code:'USD',value:'5.00'},supplementary_data:{related_ids:{capture_id:'CAPTURE123456789'}}}));assert.equal((await s.safe(()=>s.download(req('download',{token})))).status,403);await s.webhook(event('PAYMENT.CAPTURE.COMPLETED','complete2',{id:'CAPTURE123456789',supplementary_data:{related_ids:{order_id:remote.id}}}));assert.equal((await D.prepare('SELECT status FROM store_orders').first()).status,'refunded')});
test('anonymous configuration never exposes a sandbox client or checkout',async()=>{const response=await s.configuration(req('configuration',undefined,''));assert.equal(response.status,200);const c=await response.json();assert.equal(c.enabled,false);assert.equal(c.liveOpen,false);assert.equal(c.clientId,null);assert.equal(c.mode,null)});
...
test('live checkout stays closed unless explicitly enabled',async()=>{globalThis.__storeEnv.PAYPAL_MODE='live';try{assert.equal((await s.safe(()=>s.createOrder(req('create',{productIds:['college-algebra-linear'],requestKey:crypto.randomUUID()})))).status,503);assert.equal((await(await s.configuration(req('configuration',undefined))).json()).enabled,false)}finally{globalThis.__storeEnv.PAYPAL_MODE='sandbox'}});
test('invalid amounts, foreign origins and malformed JSON fail safely',async()=>{assert.throws(()=>s.cents('1.001'));assert.throws(()=>s.mutation(new Request('https://store.example/api/store/create',{headers:{origin:'https://attacker.example'}})));const malformed=new Request('https://store.example/api/store/create',{method:'POST',headers:{authorization:'owner'},body:'{broken'});assert.equal((await s.safe(()=>s.createOrder(malformed))).status,400)});
...
test('approved daily releases are listed while full files still require owner or paid access',async()=>{
 ...
 assert.equal(creates,before,'Unpaid download attempts must never reach PayPal');
});
...
test('the complete release supports protected delivery for every new product',async()=>{
 ...
 assert.equal(release.length,140);
